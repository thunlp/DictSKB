...
hownet_dir = './dataset/sememe_dict.txt'
# 用来将每个词还原到原型
...
emb_s_file = os.path.join('dataset', 'embed_sememe.pth')
if os.path.isfile(emb_s_file):
    emb_s = torch.load(emb_s_file, map_location=device)
else:
    emb_s = torch.zeros(sememe.size(), params.sememe_dim, dtype=torch.float, device=device)
...
config_nli_model = {
    ...
    'use_cuda': True,
    'sememe_dim': params.sememe_dim,
    'sememe_size': sememe.size(),
    'pool_type': params.pool_type
}

# model
encoder_types = ['LSTM_baseline', 'LSTM_concat', 'LSTM_gate', 'LSTM_cell', 'LSTM_extra_void', 'LSTM_extra_concat',
                 'LSTM_extra_gate', 'LSTM_extra_cell', \
                 'BILSTM_baseline', 'BILSTM_concat', 'BILSTM_gate', 'BILSTM_cell', 'BILSTM_extra_void',
                 'BILSTM_extra_concat', 'BILSTM_extra_gate', 'BILSTM_extra_cell', \
                 'GRU_baseline', 'GRU_concat', 'GRU_gate', 'GRU_cell', 'GRU_extra_void', 'GRU_extra_concat',
                 'GRU_extra_gate', 'GRU_extra_cell', \
                 'BIGRU_baseline', 'BIGRU_concat', 'BIGRU_gate', 'BIGRU_cell', 'BIGRU_extra_void', 'BIGRU_extra_concat',
                 'BIGRU_extra_gate', 'BIGRU_extra_cell']

print(params.encoder_type)
...
def trainepoch(epoch):
    ...
    for stidx in range(0, len(s1), params.batch_size):
        # prepare batch
        s1_batch, s1_len, s1_sememe = get_batch(s1[stidx:stidx + params.batch_size],
                                                word_vec, sememe, params.word_emb_dim)
        s2_batch, s2_len, s2_sememe = get_batch(s2[stidx:stidx + params.batch_size],
                                                word_vec, sememe, params.word_emb_dim)
        ...
        # model forward
        output = nli_net((s1_batch, s1_len, s1_sememe), (s2_batch, s2_len, s2_sememe))
...
def evaluate(epoch, eval_type='valid', final_eval=False):
    ...
    for i in range(0, len(s1), params.batch_size):
        # prepare batch
        s1_batch, s1_len, s1_sememe = get_batch(s1[i:i + params.batch_size], word_vec, sememe, params.word_emb_dim)
        s2_batch, s2_len, s2_sememe = get_batch(s2[i:i + params.batch_size], word_vec, sememe, params.word_emb_dim)
        s1_batch, s2_batch = Variable(s1_batch.cuda()), Variable(s2_batch.cuda())
