...
class BIGRU_gate(nn.Module):
    ...
    def sememe_sum(self, input_s):
        ...
        input_sememe = torch.stack(input_sememe, dim = 0)
        return input_sememe
class BIGRU_cell_baseline(nn.Module):
    def __init__(self, config):
        super(BIGRU_cell_baseline, self).__init__()
        self.enc_lstm_dim = config['enc_lstm_dim']
    ...
    def forward(self, sent, sent_len, def_vecs, device):
        sememe_h = self.sememe_sum(def_vecs)
        sent_len_sorted, idx_sort = np.sort(sent_len)[::-1], np.argsort(-sent_len)
        sent_len_sorted = sent_len_sorted.copy()
        idx_unsort = np.argsort(idx_sort)
        ...
        for time in range(max_time):
            next_hx = self.node_forward(sent[time, 0:pack_length[time]], hx_forward, sememe_h[time, 0:pack_length[time]])
            output_forward.append(torch.cat([next_hx, torch.zeros([batch_size-next_hx.size()[0], self.mem_dim], device = device)], dim = 0))
            if(time < max_time-1):
                hx_forward = next_hx[0:pack_length[time+1]]
        ...
        for time in range(max_time):
            next_hx = self.node_backward(sent[max_time-time-1, 0:pack_length[max_time-time-1]], hx_backward, sememe_h[max_time-time-1, 0:pack_length[max_time-time-1]])
            output_backward[max_time-time-1] = torch.cat([next_hx, torch.zeros([batch_size-next_hx.size()[0], self.mem_dim], device =device)], dim = 0)
            if(time < max_time-1):
                hx_backward = torch.cat([next_hx, torch.zeros([pack_length[max_time-time-2]-next_hx.size()[0], self.mem_dim]).to(device)], dim = 0)
        a = torch.stack(output_forward, dim = 0)
        b = torch.stack(output_backward, dim = 0)
        idx_unsort = torch.from_numpy(idx_unsort).to(device)
        sent_output_forward = a.index_select(1, idx_unsort)
...
class baselineModel(nn.Module):
    ...
    def forward(self, s1, s2, device):
        u = self.encoder(s1[0], s1[1], s1[2], device)
        v = self.encoder(s2[0], s2[1], s2[2], device)
