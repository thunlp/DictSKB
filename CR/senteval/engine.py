...
class SE(object):
    ...
    def test(self, name, size):
        cr = name[0]
        assert cr == 'CR'
        tpath = self.params.task_path
        self.evaluation = CREval(tpath + '/downstream/CR', seed=self.params.seed)
        self.params.current_task = cr
        self.evaluation.do_prepare(self.params, self.prepare)

        self.resultsinstance_list = self.evaluation.test(self.params, self.batcher, size)

        return self.resultsinstance_list
    ...
    def eval(self, name, size):
        # evaluate on evaluation [name], either takes string or list of strings
        if (isinstance(name, list)):
            self.results = {x: self.eval(x,size=size) for x in name}
            return self.results
        ...
        self.results = self.evaluation.run(self.params, self.batcher, size)
