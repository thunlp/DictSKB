...
import sys
sys.path.append('..')
from OpenDictNet import DictNetdict
word_candidate = {}
...
for w1, i1 in dataset.dict.items():
    ...
    if w1_s_flag == 0:
        w1_orig = w1
    try:
        # tree = dict_net.get_sememes_by_word(w1_orig, merge=False, structured=True, lang="en")
        w1_sememes = dict_net.get_sememes_by_word(w1_orig, structured=False, lang="en", merge=False)
        new_w1_sememes = [t['sememes'] for t in w1_sememes]
        # print(tree)

        w1_pos_list = dict_net.get_word_pos(w1_orig)
...
f = open('word_candidates_sense_1.pkl', 'wb')
